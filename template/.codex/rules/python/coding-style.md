...
- Use Pydantic `BaseModel` for all data structures. Prefer over dataclasses.
- All Pydantic models: `model_config = ConfigDict(frozen=True)`.
- Default to `arbitrary_types_allowed=False`. If a single file must wrap
  framework-owned types that are not Pydantic models, it may set
  `arbitrary_types_allowed=True` — but scope the exception to that one file and
  those specific types, document why, and never widen it globally. Such a model
  must stay process-local and never cross a serialization boundary (e.g. a task
  queue, cache, or database).
...
- No circular imports — dependency flows downward: the entrypoint/`api` layer →
  component packages → shared `contracts/`.
...
- No `print()` — use `structlog` with context (e.g. scope_id, session_id, component).
